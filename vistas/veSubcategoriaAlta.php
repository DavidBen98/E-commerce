<?php 
    ...
    $inactivas = "
        <form class='cont' method='POST' action='../controlador/veFuncSubcategoriaAlta.php?reactivacion=true' onsubmit='' enctype='multipart/form-data'>     
            <label for='subcategorias-inactivas' class='col-sm-2 form-label'>Reactivar subcategoría</label>
            $subcategorias_inactivas
            <div class= 'agregar'>
                <input type='submit' class='btn' name='reactivar-subcategoria' title='Reactivar subcategoria' value='Reactivar subcategoria'>    
            </div>
    ";
    ...
    if (isset($_GET["reactivacion"])){
        ...
        if ($error == "7"){
            $inactivas .= "
                <div class='contenedor mensaje' id='reactivacion'>
                    <p> Error: seleccione una subcategoría <p/>
                </div>
            ";
        }
    }
